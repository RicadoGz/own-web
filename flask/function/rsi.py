...
async def main():
    ...
    tasks = [process_stock(ticker, start_date, end_date) for ticker in company_codes]
    #  这个task 就是创建task 然后一起运行 协程
    results = await asyncio.gather(*tasks)
    
    for ticker, last_rsi in results:
        if ticker is not None and last_rsi is not None and last_rsi <= 35:
            low_rsi_stocks.append((ticker, last_rsi))
